...
<div class="row">
  <div class="col-md-12 margin-bottom-20">
    ...
    <a class="btn btn-lg blue" href="<?=base_url('webadmin/user/act/add')?>">
    ...
    <div class="wrap-status-data">
      <ul>
        <li <?=($statuspage == 'publish') ? 'class="active"' : '' ;?>>
          <a href="<?=base_url('/webadmin/user/')?>">
          Publish (<?=$count_publish?>) </a>
        </li>
        <li <?=($statuspage == 'trash') ? 'class="active"' : '' ;?>>
          <a href="<?=base_url('/webadmin/user/trash')?>">
          Trash (<?=$count_trash?>) </a>
        </li>
      </ul>
    </div>
        ...
        <tbody>
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                   Are you sure want <?=$pesan.$page?> item "<b style="font-style: italic;"><?=$row['nama']?> ?</b>"
                </div>
                <div class="modal-footer">
                  <a href="<?=base_url('webadmin/user/act/'.$fungsi.'/'.$row['id'])?>" class="btn btn-danger">Yes</a>
          ...
          <tr>
            ...
            <td align="right">
              ...
              if ($statuspage == 'trash') {
              ?>
              <a class="link-table" href="<?=base_url('webadmin/user/act/restore/'.$row['id'])?>">Restore</a>

              <a class="link-table" data-toggle="modal" style="color:red" href="#draggable<?=$row['id']?>">Delete Permanently</a>
              <?php
              } else {
              ?>
              <a href="<?=base_url('webadmin/user/act/edit/'.$row['id'])?>" class="link-table green btn btn-xs default">
